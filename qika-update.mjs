...
const DIST_HTTPS = 'https://github.com/jkboy/qika-dist.git';
const log = (m) => console.log(`[qika-update] ${m}`);

// ---------- 0. 预检:升级源可达才动服务 ----------
// 本网到 GitHub 偶发瞬断(22 端口 "Connection closed by"、代理 HTTPS 同刻失败)。npm 的 git
// 探测对此零容错:pacote 先试 HTTPS、失败静默回落 SSH,两条各一次不重试("Connection closed"
// 不在其瞬断白名单),最终日志只剩 SSH 那条。0.2.107 升级实测:服务被停 → 装失败 → 再拉起旧版,
// 一次抖动白折腾一轮。所以先探 3 次,探不通就不碰运行中的实例。`--check` 只跑预检即退出。
const remoteSha = await preflightReachable();
if (!remoteSha) {
  log('升级源(GitHub)当前不可达,未改动运行中的服务。稍后重跑 qika-update;排查网络可用 qika-update --check。');
  process.exit(2);
}
log(`升级源可达(qika-dist main ${remoteSha.slice(0, 7)})`);
...
const r = await installWithRetry(3);
if ((r.status ?? 1) !== 0) {
  log('安装失败!');
  if (isWin && /EBUSY/.test(`${r.stderr || ''}\n${r.stdout || ''}`)) reportInstallLockers();
...
// ---------- 辅助 ----------
// 预检:`git ls-remote` 探 qika-dist main,最多 3 次(间隔 2s)。走 HTTPS(与 npm 首选一致,
// 且零认证、经代理);每次 25s 超时防挂死。返回 main 的 sha,全部失败返回 null。
// 注意用 spawnSync 直调 git 而非 npm——要的是"网络是否通",不是 npm 的整套解析。
async function preflightReachable() {
  for (let i = 1; i <= 3; i++) {
    const r = spawnSync('git', ['ls-remote', DIST_HTTPS, 'refs/heads/main'], {
      encoding: 'utf8',
      timeout: 25_000,
      env: { ...process.env, GIT_TERMINAL_PROMPT: '0' },
    });
    const sha = r.status === 0 ? (r.stdout || '').trim().split(/\s+/)[0] : '';
...
// 安装重试:npm 对 git 的 "Connection closed by … port 22" 不重试(不在 @npmcli/git 的瞬断白名单),
// 这里在外层补:仅当 stderr 命中连接类文案才重试,其它失败(EBUSY、包坏、磁盘)立刻返回。
// 为拿到 stderr 判别,输出改 pipe 后原样转发,用户看到的日志不变。
async function installWithRetry(maxAttempts) {
  const transient = /Connection closed by|Connection reset|Connection timed out|remote end hung up|Could not resolve host|Failed to connect|Recv failure|TLS connect error|error: RPC failed|code 128/i;
  let r;
  for (let i = 1; i <= maxAttempts; i++) {
    r = spawnSync('npm', ['install', '-g', DIST_URL], { encoding: 'utf8', shell: isWin, maxBuffer: 64 * 1024 * 1024 });
    if (r.stdout) process.stdout.write(r.stdout);
    if (r.stderr) process.stderr.write(r.stderr);
    if ((r.status ?? 1) === 0) return r;
    const text = `${r.stderr || ''}\n${r.stdout || ''}`;
    if (i < maxAttempts && transient.test(text)) {
      const wait = 3000 * i;
      log(`安装遇网络瞬断(第 ${i}/${maxAttempts} 次),${wait / 1000}s 后重试…`);
      await sleep(wait);
      continue;
    }
    return r;
...
// 找会锁住安装目录的浏览器接管进程,两路并集:
// ①命令行或映像路径在全局安装的 native-host 下——不限 node.exe:Chrome 拉 host 的 cmd.exe(host.cmd)、
//   bridge 常驻的 PowerShell 助手(win32-window.ps1)、native-host\node.exe 都算;
// ②占着 bridge 端口 9204/9205 的 node.exe,只要命令行不是源码仓库布局——手动 `node bridge.js`
//   相对路径启动的孤儿 bridge 命令行里没有安装路径,①扫不到,cwd 却钉在安装目录里
//   (2026-09-26 实测:装到一半 EBUSY rename native-host)。
// 不误伤源码仓库开发实例:其路径是 pi-web\packages\browser-takeover\...,两路都不匹配。
function findNativeHostPids() {
  const script = String.raw`
$re = 'pi-web[\\/]browser-takeover[\\/]native-host'
$dev = 'packages[\\/]browser-takeover'
$ids = @(Get-CimInstance Win32_Process | Where-Object { $_.ProcessId -ne $PID -and ($_.CommandLine -match $re -or $_.ExecutablePath -match $re) } | ForEach-Object { $_.ProcessId })
foreach ($p in @(Get-NetTCPConnection -State Listen -LocalPort 9204,9205 -ErrorAction SilentlyContinue | ForEach-Object { $_.OwningProcess })) {
  $w = Get-CimInstance Win32_Process -Filter "ProcessId=$p"
  if ($w -and $w.Name -eq 'node.exe' -and $w.CommandLine -notmatch $dev) { $ids += $p }
}
...
// EBUSY 诊断:列出命令行/映像路径落在全局安装包里的进程。cwd 停在安装目录里的进程
// (终端、资源管理器窗口)在命令行里看不出来,只能提示用户自查。
function reportInstallLockers() {
  const script = String.raw`
Get-CimInstance Win32_Process | Where-Object { $_.ProcessId -ne $PID -and ($_.CommandLine -match 'node_modules[\\/]pi-web[\\/]' -or $_.ExecutablePath -match 'node_modules[\\/]pi-web[\\/]') } | ForEach-Object { '{0} {1} {2}' -f $_.ProcessId, $_.Name, $_.CommandLine }`;
  let lines = [];
  try {
    lines = execFileSync('powershell', ['-NoProfile', '-Command', script], { encoding: 'utf8' })
      .split(/\r?\n/)
      .map((l) => l.trim())
      .filter((l) => l && !/qika-update/.test(l));
  } catch {
    /* 诊断失败不影响恢复路径 */
  }
  log('安装目录被占用(EBUSY)。可能的占用者:');
  if (lines.length) for (const l of lines.slice(0, 10)) log(`  ${l.slice(0, 200)}`);
  else log('  命令行里没找到——多半是某个终端/资源管理器窗口的当前目录停在安装目录里');
  log('结束占用者(或关掉那个窗口)后重跑 qika-update。');
}
