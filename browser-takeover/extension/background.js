...
//   快照/观察：snapshot / getText / queryAll / pageInfo / windowState（窗口可见性，bridge 截图前用）
...
async function handleCommand(msg) {
  switch (msg.cmd) {
    ...
    case "windowState": return windowState(msg.tabId);
    // 导航 / 标签
...
// windowState：目标标签所在窗口的可见性事实，供 bridge 决定截图前要不要临时还原窗口。
// visibility 取页面自己的 document.visibilityState：窗口最小化、被别的窗口完全遮挡、
// 标签不在前台，Chromium 一律报 hidden——hidden 的页面不出帧，CDP 截图会一直等不到画面。
// windowTitle 是该窗口前台标签的标题（OS 窗口标题 = 它 + " - Google Chrome"），原生侧靠它定位窗口。
async function windowState(tabId) {
  tabId = await resolveTabId(tabId);
  const tab = await chrome.tabs.get(tabId);
  const win = await chrome.windows.get(tab.windowId);
  const [front] = await chrome.tabs.query({ windowId: win.id, active: true });
  const visibility = await evaluate(tabId, "document.visibilityState").catch(() => null);
  return {
    tabId,
    windowId: win.id,
    state: win.state,
    ...
    windowTitle: front?.title ?? tab.title ?? "",
    visibility,
  };
}
...
async function withDebugger(tabId, fn) {
  ...
  try {
    // 页面不在前台（窗口最小化/被遮挡/标签在后台）时 Chromium 不给它焦点：focus 事件不触发、
    // 惰性初始化的编辑器不激活，受信任输入"打了等于没打"。仿真焦点让后台页面按前台语义响应
    // （Playwright 对 Chromium 页面默认同样开启）。detach 时随 session 一并清除。
...
const HIDDEN_CAPTURE_TIMEOUT_MS = 2500;
const VISIBLE_CAPTURE_TIMEOUT_MS = 15000;
// 给模型看的失败原因：说清是页面不可见，并堵住"把浏览器窗口拉到前台"这条歧路
// （曾有会话用 ShowWindow/SetForegroundWindow 脚本还原窗口来救截图，把用户正在用的前台抢走）。
const NOT_VISIBLE_HINT =
  "截图失败：页面当前不可见（浏览器窗口最小化或被完全遮挡时 Chromium 不渲染画面）。" +
  "不要去还原、激活或改动浏览器窗口——改用 snapshot/getText/evaluate 验证结果，或提醒用户把浏览器窗口露出一角后再截图。";
...
async function screenshot(tabId, opts = {}) {
  ...
  try {
    // 整个窗口不可见（最小化/被完全遮挡）时切标签也救不了：页面仍是 hidden、永远不出帧，
    // captureScreenshot 会无限挂起——不守卫就是 CLI 侧 20s 超时 + 一条把锅推给 bridge 的误报。
    // 先问页面自己的 visibilityState 定超时：hidden 只等一小段（帧可能正好在路上），visible 给足余量。
    const visibility = await evaluate(tabId, "document.visibilityState").catch(() => null);
    const timeoutMs = visibility === "hidden" ? HIDDEN_CAPTURE_TIMEOUT_MS : VISIBLE_CAPTURE_TIMEOUT_MS;
