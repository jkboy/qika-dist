...
**QiKa Code 0.2.128 起由安装器自动完成**：`qika-update` / 首次安装结束时调用的 `post-install.mjs` 会把包内 `pi-agent/` 同步到 pi 的 agent 目录
（`PI_CODING_AGENT_DIR` 或 `~/.pi/agent`）——目标缺失则写入、内容一致则跳过、本机改过则先备份为 `.bak-<时间>` 再覆盖，不删目录里的其他文件。
装完**重启 QiKa Code** 新会话即用新工具（pi 的扩展加载有进程级缓存，不重启同项目新会话仍是旧代码）。

下面的手动步骤只在没走安装器（源码开发 / 自定义 agent 目录）时需要：
