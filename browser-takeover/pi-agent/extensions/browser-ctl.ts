...
const browserCtlTool = defineTool({
  ...
  promptGuidelines: [
    ...
    "The browser window may stay minimized or behind other windows: snapshot/click/fill/evaluate work there, and screenshot briefly un-minimizes the window without stealing focus and re-minimizes it afterwards. Never restore, activate, resize or focus the browser window yourself (no ShowWindow/SetForegroundWindow scripts) — if screenshot reports the page is not visible, verify with snapshot/getText instead or tell the user.",
  ],
