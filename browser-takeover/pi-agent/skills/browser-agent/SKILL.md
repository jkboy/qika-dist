...
- **浏览器窗口可以一直最小化 / 压在别的窗口后面**：snapshot/click/fill/evaluate 在后台照常工作（扩展已开焦点仿真，后台页面的 focus 事件正常触发）；`screenshot` 会由 bridge 在截图瞬间**不抢焦点**地把窗口露一下、截完立刻放回最小化。**绝对不要自己去还原、激活、缩放浏览器窗口**（ShowWindow/SetForegroundWindow 脚本、`chrome.windows` 技巧都不行）——那会把用户正在用的前台窗口抢走。若 `screenshot` 报"页面当前不可见"，改用 snapshot/getText 验证，或直接告知用户。
