...
4. **等待** —— 页面加载、元素出现用 `waitFor`（比裸 sleep 可靠）：
   ...
   ```
5. **验证** —— 每次操作后用 `getText`/`evaluate`/`screenshot` 核对结果，再决定下一步。
6. **后续步骤已知时一次发完（`steps`）** —— 「操作 → 等待 → 验证」这种你已经知道要做什么的序列，不要拆成三轮，用 `action="steps"` 一次顺序执行；任一步失败立即停止，返回已完成步骤的全部结果 + 失败原因 + 未执行的步骤：
   ```
   browser_ctl(action="steps", steps=[
     {action:"click", selector:"e3"},
     {action:"waitFor", options:{selector:".result", state:"visible", timeout:8000}},
     {action:"getText", selector:".result"}
   ])
   ```
   只有「下一步取决于页面显示什么」时才拆成单独调用（比如先 snapshot 再决定点哪个）。最多 20 步；steps 里不能再嵌 steps。
...
| `pressKey` | `key` | 按键 |
| `scroll` | `options` | 滚动 |
| `waitFor` | `options={selector 或 fn, state, timeout}` | 条件等待（state=visible/hidden/removed）；只给 `timeout` 不给条件 = 纯延时（最多 20s） |
| `screenshot` | `options` | 截图；`options={fullPage:true}` 全页，`options={selector}` 元素级 |
| `evaluate` | `expression` | 任意 JS（逃生舱） |
| `steps` | `steps=[{action, ...}, ...]` | **顺序执行多步**，首败即停，返回每步结果；每项参数与单动作相同，≤20 步 |
...
- **浏览器窗口可以一直最小化 / 压在别的窗口后面**：snapshot/fill/evaluate 在后台照常工作（扩展已开焦点仿真，后台页面的 focus 事件正常触发）。**但窗口最小化时视口是 0×0，受信任 `click` 会打在 (0,0) 落空**（返回的 x/y ≤ 0 就是这个信号，2026-09-18 实测；被遮挡但未最小化时正常）——点了之后要验证效果，没触发就改用 `evaluate` 的 `el.click()`；`screenshot` 会由 bridge 在截图瞬间**不抢焦点**地把窗口露一下、截完立刻放回最小化。**绝对不要自己去还原、激活、缩放浏览器窗口**（ShowWindow/SetForegroundWindow 脚本、`chrome.windows` 技巧都不行）——那会把用户正在用的前台窗口抢走。若 `screenshot` 报"页面当前不可见"，改用 snapshot/getText 验证，或直接告知用户。
- **多步任务**：后续步骤已知就用 `steps` 一次发（首败即停会告诉你停在第几步、已完成的步骤结果都在），把「验证」放在同一个 steps 的最后一步；只有下一步取决于页面内容时才拆轮。工具失败（找不到元素、waitFor 超时、扩展报错）会**直接报错**，不会返回空结果——看到报错就别当成功。

## 面向模型的设计说明

- 模型看到的工具面：`browser_ctl` 一个工具 + action 枚举 + 少量可选参数 + `steps[]`（元素 schema 与单动作同）。`promptSnippet`/`promptGuidelines` 常驻系统提示。
- Token 影响：工具 schema 固定（每请求一小段；steps 子 schema 约 +60 token）；`snapshot` 结果含元素清单 + 正文（可到数百 token），`queryAll` 可限量；一次 steps 的结果 = 各步结果之和，替代原本 N 轮各自的结果与 N−1 次完整请求。
- KV Cache：工具面固定，无按 turn 变化的前缀；追加式稳定（schema 变更只在发版时打一次缓存）。
- 归类：这是**补下限**机制（把"操作真实浏览器"这个能力补上，不约束模型发挥），对更强模型同样成立，可放心长期使用。
- **`steps` 同样是补下限（E 类经济学）**：本机 51 个会话里 602 次「机械动作 → 紧跟一轮针对性验证」各花两轮、p50 10.6s，其中 79% 的验证内容模型在动作前就已写好——步骤已知却被工具面强制拆轮，是与模型强弱无关的等待成本。steps 不替模型决定做什么，每步仍由模型显式指定；用不用由模型判断。验收仪表 `node scripts/browser-turn-stats.mjs`（`mechToObservePairs` 应下降）。
...
- **最小化窗口下工作**（焦点仿真 + 截图时不抢焦点地临时露窗 + 不可见守卫）也是补下限：修的是宿主/渠道层的失败源——Chromium 对不可见页面不出帧、不派发焦点，与模型无关；此前模型在截图超时后自行 ShowWindow 还原窗口，是被误导性的超时文案（"bridge 可能未就绪"）推向的歧路。guidelines 再多一句（~70 token 固定开销）。
