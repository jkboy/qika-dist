...
| `screenshot` 报"页面当前不可见" | 浏览器窗口最小化/被完全遮挡且 bridge 侧的窗口还原没成功（非 Windows、或 PowerShell 助手被策略禁用）。bridge 日志找 `win32 helper`；agent 不应自行还原窗口，用 snapshot/getText 代替或让用户把窗口露出一角 |
| 截图时浏览器窗口闪一下 | 正常：窗口最小化时 bridge 会不抢焦点地短暂还原再最小化回去（约 0.3s），焦点始终留在用户当前窗口。只会动**装了扩展的那个浏览器**（从 host 进程父链定位），机器上其它浏览器不受影响 |
