...
│  ├─ visible-capture.js         #   截图编排：窗口最小化/被遮挡时不抢焦点地临时露窗再放回
│  ├─ win32-window.js / .ps1     #   Windows 窗口显隐助手（常驻 PowerShell，user32 无激活操作；只碰拉起 host 的那个浏览器进程）
...
③④⑤ 这类**后续步骤已知**的序列，`browser_ctl` 工具面提供 `action=steps` 一次顺序执行（首败即停、返回每步结果），
省掉每步一轮模型往返；CLI 侧对应的是 `drive.js` 的脚本文件模式。只有下一步取决于页面内容时才逐步走。
